...
function BrandLockup({ locale }: { locale: Locale }) {
  return (
    <Link
      ...
      aria-label={t(locale, "NOVEM.sk, späť na hlavnú stránku")}
    >
      <Image
        src="/logo.png"
        alt="NOVEM.sk"
...
function ReferenceLink({ name, href, locale }: { name: string; href?: string; locale: Locale }) {
  ...
  const content = (
    <>
      ...
      {href !== "/" && (
        <>
          <span className="sr-only"> {t(locale, "(otvorí sa v novej karte)")}</span>
          <ArrowUpRight
            aria-hidden="true"
  ...
  if (href === "/") return <Link href={`/${locale}`} className={className}>{content}</Link>
...
export function OdooLanding({ locale }: { locale: Locale }) {
  return (
      ...
      <header className="px-4 sm:px-6 lg:px-8">
        <div className="mx-auto flex max-w-7xl items-center justify-between border-b border-slate-900/15 py-5">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <Button asChild variant="outline" className="hidden h-11 rounded-full border-slate-900/20 bg-transparent sm:inline-flex">
              <Link href={`/${locale}#kontakt`}>{t(locale, "Porozprávať sa")}</Link>
            </Button>
          </div>
        </div>
      </header>
      ...
      <main id="main-content">
          ...
          <div className="mx-auto max-w-7xl">
            <div className="grid gap-12 lg:grid-cols-[1.08fr_.92fr] lg:items-end">
              <div>
                ...
                <h1 className="max-w-4xl text-balance text-5xl font-semibold leading-[1.02] tracking-[-0.05em] sm:text-7xl lg:text-[5.6rem]">
                  {t(locale, "Odoo zavádzame tak, ako vedieme firmu. Osobne.")}
                </h1>
                <p className="mt-7 max-w-2xl text-pretty text-lg leading-8 text-slate-600">
                  {t(locale, "Sme rodinná firma Juríčkovcov. Od roku 2016 riešime technológie našich klientov a od roku 2025 aj Odoo, ktoré sme si najprv poctivo nasadili u seba.")}
                </p>
              </div>
              <figure className="relative isolate aspect-[4/3] overflow-hidden rounded-[2rem] bg-[#183544] text-white shadow-xl shadow-slate-900/10">
                <Image src="/novem-team.webp" alt={t(locale, "Tím NOVEM v kancelárii")} fill priority sizes="(min-width: 1024px) 42vw, 100vw" className="object-cover" />
                ...
                <Badge className="absolute left-5 top-5 border-white/25 bg-white/90 text-slate-950 shadow-sm hover:bg-white/90">{t(locale, "Tím NOVEM")}</Badge>
                <figcaption className="absolute inset-x-0 bottom-0 p-6 sm:p-8">
                  <p className="max-w-md text-balance text-2xl font-medium leading-tight sm:text-3xl">{t(locale, "Nie sme korporát. Klient pozná ľudí, ktorí jeho riešenie naozaj robia.")}</p>
                </figcaption>
              </figure>
            </div>
            <div className="mt-10 flex flex-col gap-3 sm:flex-row">
              ...
              <Button asChild size="lg" variant="ghost" className="h-12 rounded-full px-6">
                <a href="#pribeh">{t(locale, "Prečítať náš príbeh")} <ArrowRight data-icon="inline-end" aria-hidden="true" /></a>
              </Button>
            </div>
          </div>
        </section>
        ...
        <section id="pribeh" className="scroll-mt-20 border-y border-slate-900/15 bg-white/45 px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto grid max-w-7xl gap-16 lg:grid-cols-[.72fr_1.28fr]">
            <div><p className="font-mono text-sm text-primary">{t(locale, "2016 → dnes")}</p><h2 className="mt-4 text-4xl font-semibold tracking-tight">{t(locale, "Technický partner, ktorý zostáva pri stole.")}</h2></div>
            <ol className="border-l border-slate-900/20">
              {[
                ["2016", t(locale, "Začíname ako rodinná IT firma"), t(locale, "Budujeme prax v podpore, sieťach, bezpečnosti a outsourcingu technológií.")],
                ["2025", t(locale, "Odoo nasadzujeme pre NOVEM"), t(locale, "Procesy si overujeme na vlastnej firme skôr, než ich ponúkneme klientom.")],
                [t(locale, "Dnes"), t(locale, "Odoo prepájame s celým IT"), t(locale, "Implementáciu nekončíme odovzdaním. Na riešenie nadväzuje technická podpora a bezpečná prevádzka.")],
              ].map(([year, title, text]) => (
        ...
        <section id="tim" className="scroll-mt-20 px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-7xl">
            <div className="mb-10 max-w-2xl"><p className="text-sm font-semibold uppercase tracking-[0.16em] text-primary">{t(locale, "Ľudia za riešením")}</p><h2 className="mt-3 text-4xl font-semibold tracking-tight">{t(locale, "Malý tím. Jasné kompetencie.")}</h2></div>
            <div className="grid gap-x-8 sm:grid-cols-3">
              {[
                ["2", t(locale, "certifikovaní Odoo špecialisti"), t(locale, "Ovládame najnovšie funkcie a možnosti platformy Odoo.")],
                ["4", t(locale, "technici zákazníckej podpory"), t(locale, "Každodenná pomoc pri prevádzke a incidentoch.")],
                ["2", t(locale, "špecialisti na siete a bezpečnosť"), t(locale, "Infraštruktúra, monitoring a ochrana dát.")],
        ...
        <section id="referencie" className="scroll-mt-20 bg-[#183544] px-4 py-20 text-white sm:px-6 lg:px-8">
          <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[.6fr_1.4fr]">
            <div><p className="text-sm text-sky-200">{t(locale, "Dlhodobá IT prax")}</p><h2 className="mt-3 text-4xl font-semibold tracking-tight">{t(locale, "Referencie")}</h2></div>
            <div className="grid grid-cols-2 gap-x-8 border-t border-white/20 sm:grid-cols-3">
              {references.map((reference, index) => <div key={reference.name} className="flex min-h-14 items-center border-b border-white/15 py-1 text-sm"><span className="mr-3 font-mono text-[10px] text-sky-300 tabular-nums">{String(index + 1).padStart(2, "0")}</span><ReferenceLink {...reference} locale={locale} /></div>)}
            </div>
          </div>
        </section>
        ...
        <section className="px-4 py-20 sm:px-6 lg:px-8">
          <div className="mx-auto flex max-w-7xl flex-col justify-between gap-8 border-b border-slate-900/15 pb-20 sm:flex-row sm:items-end">
            <h2 className="max-w-3xl text-balance text-4xl font-semibold tracking-tight sm:text-5xl">{t(locale, "Dobré ERP sa začína normálnym rozhovorom o tom, ako firma funguje.")}</h2>
            <ContactLink locale={locale} />
          </div>
        </section>
      </main>
